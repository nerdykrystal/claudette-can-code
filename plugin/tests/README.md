...
Stage 03/04/05 (Haiku) will fill actual test names in the "Test File / Test Name" column. This table is the authoritative mapping from requirements to tests for the CDCC Plugin MVP.

## Functional Requirements (FR)

| Requirement | Description | Test File | Test Name | Status |
|-------------|-------------|-----------|-----------|--------|
| FR-001 | Bundle Consumer reads 4-doc bundle and asserts approval status | tests/unit/bundle-consumer.test.ts | — | Stage 03 |
| FR-002 | Bundle Consumer returns tagged error on missing or unapproved doc | tests/unit/bundle-consumer.test.ts | — | Stage 03 |
| FR-003 | Plan Generator assigns a model to every stage | tests/unit/plan-generator.test.ts | — | Stage 03 |
| FR-004 | Plan Generator assigns effort level to every stage | tests/unit/plan-generator.test.ts | — | Stage 03 |
| FR-005 | Plan Generator assigns spec depth to every stage | tests/unit/plan-generator.test.ts | — | Stage 03 |
| FR-006 | Plan Generator output is deterministic on same input | tests/unit/plan-generator.test.ts | — | Stage 03 |
| FR-007 | Plan artifact validates against plan.schema.json | tests/unit/plan-writer.test.ts | — | Stage 03 |
| FR-008 | Plan artifact is fsync'd on write | tests/unit/plan-writer.test.ts | — | Stage 03 |
...
| FR-017 | Your Setup Catalog gracefully returns empty on missing settings.json | tests/unit/catalog.test.ts | — | Stage 03 |
| FR-018 | Skill-Gap Checker returns missing skills | tests/unit/skill-gap-checker.test.ts | — | Stage 03 |
